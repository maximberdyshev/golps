...
func (i *NoItemState) requestItem(bot WebAPI, update tgbotapi.Update) {
...
func (i *NoItemState) addItem(count int, bot WebAPI, update tgbotapi.Update) {
...
func (i *NoItemState) insertMoney(money int, bot WebAPI, update tgbotapi.Update) {
	msgTxt := fmt.Sprintf("Осталось предметов: %v", i.vendingMachine.GetItemCount())
	markup := BackInlineMarkup
	if err := bot.EditTextAndMarkup(entity.Message{
		ChatID:    update.CallbackQuery.From.ID,
...
func (i *NoItemState) dispenseItem(bot WebAPI, update tgbotapi.Update) {
	msgTxt := fmt.Sprintf("Осталось предметов: %v", i.vendingMachine.GetItemCount())
	markup := BackInlineMarkup
	if err := bot.EditTextAndMarkup(entity.Message{
		ChatID:    update.CallbackQuery.From.ID,
...
func (i *NoItemState) exit(bot WebAPI, update tgbotapi.Update) {
